...
function deriveCounterparty(
  ...
  if (dealType === "LEASE") {
    return (
      fieldString(fields, "counterparty") ??
      fieldString(fields, "lessee") ??
      fieldString(fields, "lessee_entity")
    );
  }
  return (
    fieldString(fields, "counterparty") ??
    fieldString(fields, "buyer") ??
    fieldString(fields, "buyer_entity")
  );
}
...
export function mapExtractionToDocumentFields(
  ...
  if ("sections" in extraction && extraction.documentType === "OLA") {
    ...
    if (parties.counterparty) metadata.counterparty = parties.counterparty as FieldValue;
  ...
  let counterparty = deriveCounterparty(dealType, metadata);

  if (!counterparty && dealType === "LEASE") {
    counterparty = getString("lessee") ?? getString("lessee_entity");
  } else if (!counterparty && dealType === "PURCHASE") {
    counterparty = getString("buyer") ?? getString("buyer_entity");
  }

  if (counterparty && !metadata.counterparty) {
    metadata.counterparty = { value: counterparty, confidence: 0.8 };
  }

  if (dealType === "LEASE" && documentType === "LOI") {
    const lessee = getString("lessee") ?? counterparty;
    if (lessee && !metadata.lessee) {
      metadata.lessee = { value: lessee, confidence: 0.8 };
    }
  } else if (dealType === "PURCHASE" && documentType === "LOI") {
    const buyer = getString("buyer") ?? counterparty;
